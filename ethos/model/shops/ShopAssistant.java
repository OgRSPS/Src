...
public class ShopAssistant {
	...
	public void buyFromShopPrice(int removeId, int removeSlot) {
		...
		if (c.myShopId == 9) {
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs [ @blu@" + getSpecialItemValue(removeId) + " @bla@ ] Donator scrolls.");
			return;
		}
		if (c.myShopId == 18) {
	...
	public boolean sellItem(int itemID, int fromSlot, int amount) {
		...
		if (amount > 0 && itemID == (c.playerItems[fromSlot] - 1)) {
			...
			//for (int i = amount; i > 0; i--) {
				...
				if (c.myShopId != 26 && c.myShopId != 44) {
					TotPrice2 *= 0.667;
				}
	...
	public void handleOtherShop(int itemID, int amount) {
		...
		if (c.myShopId == 12) {
			...
			int itemValue = getSpecialItemValue(itemID) * amount;
			if (!c.getItems().playerHasItem(6802, itemValue)) {
				c.sendMessage("You do not have enough Donator scrolls to buy this item.");
				return;
			} else {
				c.getItems().deleteItem(6802, itemValue);
				c.getItems().addItem(itemID, amount);
