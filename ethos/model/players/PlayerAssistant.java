...
public class PlayerAssistant {
	...
	public void destroyInterface(String config) {// Destroy item created by Remco
		...
		String[][] info = { // The info the dialogue gives
				...
				{ "simply type ::toggledrop on/off", "14183" }, { itemName, "14184" } };
	...
	public void sendString2( final int id, final String s) {
		if (c.getOutStream() != null && c != null) {
			c.getOutStream().createFrameVarSizeWord(126);
			c.getOutStream().writeString(s);
			c.getOutStream().writeWordA(id);
	...
	public void sendFrame16999(int id, String s) {
		if (s == null) {
			return;
		}
		if (!c.checkPacket126Update(s, id) && !s.startsWith("www")) {
	...
	public void alchemy(int itemId, String alch) {
		...
		case "low":
			if (System.currentTimeMillis() - c.alchDelay > 1000) {
				...
				sendFrame106(6);
				addSkillXP(MagicData.MAGIC_SPELLS[49][7] * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : 125), 6,
						true);
				refreshSkill(6);
			}
			break;
		...
		case "high":
			if (System.currentTimeMillis() - c.alchDelay > 2000) {
				...
				sendFrame106(6);
				addSkillXP(MagicData.MAGIC_SPELLS[50][7] * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : 125), 6,
						true);
				refreshSkill(6);
			}
			break;
						...
						if (c.inWild()) {
							...
							if (!playerKiller.getPlayerKills().killedRecently(c.connectedFrom)
								...
								if (playerKiller.inClanWars() || playerKiller.inClanWarsSafe()) {
									playerKiller.sendMessage(
											"You are not rewarded for your efforts, as you have recently defeated"
	...
	public void giveLife() {
		...
		if (!c.inDuelArena() && !Boundary.isIn(c, Boundary.DUEL_ARENA)&& !VorkathInstance.inVorkath(c)
	...
	public void startTeleport(int x, int y, int height, String teleportType, boolean homeTeleport) {
		...
		if (!c.isDead && c.teleTimer == 0 && c.respawnTimer == -6) {
			...
			if (teleportType.equalsIgnoreCase("modern") || teleportType.equals("glory")) {
				...
				c.teleTimer = 13;
				c.teleEndAnimation = 65535;
			} else if (teleportType.equals("keymaster")) {
				...
				for (int i = 0; i < loop - (k == 0 ? 1 : 0); i++) {
					...
					if (Misc.distance(x3, y3, x, y) < lowDist) {
						if (!projectile && PathChecker.isMeleePathClear(x3, y3, z, otherX, otherY)
							...
							if (PathFinder.getPathFinder().accessable(c.getX(),c.getY(),c.getHeight(), x3, y3)) {
		...
		if (otherX == c.absX && otherY == c.absY) {
			...
			for (int[] nondiag : nondiags) {
				...
				if (lowDist == 0 || Misc.distance(c.absX, c.absY, x2, y2) < lowDist) {
					if (PathFinder.getPathFinder().accessable(c.absX,c.absY,c.heightLevel, x2, y2)) {
						lowX = x2;
						lowY = y2;
	...
	public void levelUp(int skill) {
		...
		if (c.maxRequirements(c) && !c.getRights().isOrInherits(Right.OWNER)) {
			PlayerHandler
					.executeGlobalMessage("@red@" + Misc.capitalize(c.playerName) + " has reached max total level!");
		}
		if (getLevelForXP(c.playerXP[skill]) == 99 && !c.getRights().isOrInherits(Right.OWNER)) {
	...
	public boolean addSkillXP(int amount, int skill, boolean dropExperience) {
		...
		if(skill == Skill.DEFENCE.getId() 
			...
			/*if(c.playerLevel[skill] < 10)
				amount *= 3.0;
			if(c.playerLevel[skill] < 25)
				amount *= 2.5;
			if(c.playerLevel[skill] < 50)
				amount *= 2.0;
			if(c.playerLevel[skill] < 75)
				amount *= 2.0;*/
		}
		if(Config.bonusXP) {
			amount *= 1.5;
		...
		if(amount > 0) {
			sendExperienceDrop(true, amount, skill);
		}
	...
	public void addStarter() {
		PlayerHandler.executeGlobalMessage("[@blu@New Player@bla@] " + Misc.capitalizeJustFirst(c.playerName) + " @bla@has logged in for the first time! Welcome!");
		c.sendMessage("@blu@For additional information about the game, reference the manual in your inventory!");
		c.sendMessage("@blu@It has tons of useful information.");
		...
		//}
			...
			c.getItems().wearItem(1327, 1, c.playerWeapon);
			c.getItems().wearItem(1731, 1, c.playerAmulet);
			c.getItems().wearItem(6106, 1, c.playerFeet);
			c.getItems().wearItem(6110, 1, c.playerHands);
			c.getItems().wearItem(1052, 1, c.playerCape);
	...
	public static void noteItems(Player player, int item) {
		...
		if (definition == null || notedDefinition == null
			...
			player.sendMessage("You cannot note this item.");
			return;
		}
	 ...
	 * @param widgetId
	 * 
	 * NPCId is sent to the client
	 * WidgetID Is also sent to the client
	 * 231 = Packet ID
	 */
	
	public void sendNpcIdToDisplayPacket(int npcId, int widgetId) {
		if (Objects.nonNull(c.getOutStream())) {
			c.getOutStream().createFrame(231);
			c.getOutStream().writeWord(npcId);
			c.getOutStream().writeWord(widgetId);
			c.flushOutStream();
		}
	}
