...
import ethos.model.content.teleportation.TeleportTabTeleHandler;
...
public class Player extends Entity {
	...
	private TeleportInterface.TeleportInterfaceData tid;
	...
	public TeleportInterface.TeleportInterfaceData getTeleportInterfaceData() {
		return tid;
	}

	public void setTeleportInterfaceData(TeleportInterface.TeleportInterfaceData tid) {
		this.tid = tid;
	}
	...
	private GalvekBag galvekBag = new GalvekBag(this);
	...
	public int raidPoints, unfPotHerb, unfPotAmount, wrenchObject = 0, halloweenOrderNumber = 0, speed1 = -1,
			...
			betsWon, betsLost, slayerPoints, playTime, killStreak, day, month, YEAR, totalLevel, xpTotal,
			...
			delayedDamage2 = 0, delayedDamage3 = 0, pcPoints = 0, donatorPoints = 0, magePoints = 0, lastArrowUsed = -1, clanId = -1,
	...
	private int chatTextColor = 0, chatTextEffects = 0, dragonfireShieldCharge, runEnergy = 100, lastEnergyRecovery,
			...
			toxicTridentCharge, arcLightCharge, runningDistanceTravelled, trivaPoints, interfaceOpen;
	...
	public boolean teleportingToDistrict = false, morphed = false, isIdle = false, boneOnAltar = false,
			...
			recoverysSet, protectItem = false, doubleHit, tripleHit, usingSpecial, npcDroppingItems, usingRangeWeapon, usingBow,
			...
			boltTips = false, arrowTips = false, javelinHeads = false, wogw = false, sendServerPackets = false, hasSacrificedFcape = false;
	...
	public long wogwDonationAmount, lastAuthClaim, totalGorillaDamage, totalGorillaHitsDone, totalMissedGorillaHits,
			...
			lastMarkDropped, lastObstacleFail, lastForceMovement, lastAgilAction, lastDropTableSelected, lastDropTableSearch,
			...
			wogwDelay = System.currentTimeMillis(), lastChinMulti = System.currentTimeMillis(), lastBarrageMulti = System.currentTimeMillis();
	...
	public void destruct() {
		...
		if (VorkathInstance.inVorkath(this)) {
		...
		disconnected = true;
		if(!getRights().isOrInherits(Right.OWNER)) {
			if (getMode().isOsrs()) {
				com.everythingrs.hiscores.Hiscores.update("a53696abmx8f36eshjya54s4ih9wit72xuo4efwfpcgs6ecdinmuy8hhjzq7hew6ziq1y30udi", "OSRS", this.playerName, this.playerRank, this.playerXP, debugMessage);
			} else {
				if(getMode().isIronman()) {
					com.everythingrs.hiscores.Hiscores.update("a53696abmx8f36eshjya54s4ih9wit72xuo4efwfpcgs6ecdinmuy8hhjzq7hew6ziq1y30udi", "Ironman", this.playerName, this.playerRank, this.playerXP, debugMessage);
				}
				com.everythingrs.hiscores.Hiscores.update("a53696abmx8f36eshjya54s4ih9wit72xuo4efwfpcgs6ecdinmuy8hhjzq7hew6ziq1y30udi", "Normal Mode", this.playerName, this.playerRank, this.playerXP, debugMessage);
			}
		}
	...
	public String bankValueCalc(Player c) { 
		...
		if(bankValue >= 1000000) { //millies
			bankValueString = String.format("@gre@%.2fM", bankValue/ 1000000.0);
		}
		if(bankValue >= 1000000000) { //billies
			bankValueString = String.format("@yel@%.2fB", bankValue/ 1000000000.0);
		}
	...
	public String getTotalXP(Player c) {
		int total = 0;
		String xpString = Integer.toString(total);
		for(int i=0; i<23; i++) {
			total += c.playerXP[i];
		}
		xpString = Integer.toString(total);
		if(total >= 1000000) { //millies
			xpString = String.format("@gre@%.2fM", total/ 1000000.0);
		}
		if(total >= 1000000000) { //billies
			xpString = String.format("@yel@%.2fB", total/ 1000000000.0);
		}
		return xpString;
	}
	...
	private enum RankUpgrade {
		CONTRIBUTOR(Right.CONTRIBUTOR, 10), SPONSOR(Right.SPONSOR, 50), SUPPORTER(Right.SUPPORTER, 100), VIP(
				Right.DONATOR, 250), INFAMOUS(Right.SUPER_DONATOR,
						500), LEGENDARY(Right.EXTREME_DONATOR, 1000), MYTHICAL(Right.LEGENDARY, 2500);
	...
	public void initialize() {
		try {
			...
			setSidebarInterface(13, 47500); //47500

			handleTeleport.loadMonsterTab();
			...
			//future ref since i added a string to right enum just check if they aren't a player and add a boolean if they should be announced or not.
			//never fucking add developer to this.
			if(getRights().isOrInherits(Right.MODERATOR) || getRights().isOrInherits(Right.HELPER) ||
				getRights().isOrInherits(Right.ADMINISTRATOR) || getRights().isOrInherits(Right.OWNER)) {
				for (Right right : Right.values()) {
					if(right == getRights().getPrimary())
						if(!playerName.equalsIgnoreCase("Og"))
							PlayerHandler.executeGlobalMessage("@red@" + right.getTitle() + " " + playerName + " has logged in!");
				}
			}
	...
	public void updateQuestTab(){
		//getPA().setConfig(10404, 1);
		//getPA().sendConfig(10404, 1);
		
		questTab = 0;
		String totalXP = this.getTotalXP(this);
		String bankValue = this.bankValueCalc(this);
		String invValue = this.getItems().getTotalWorthSTR();
		...
		String xpString = "";
		if(Config.BONUS_WEEKEND || Config.BONUS_XP_WOGW || Config.bonusXP) {
			xpString = "@gre@Active";
		} else 
			xpString = "@red@Inactive";
		...
		if(this.dailyEnabled) {
			...
			dailyStatus = "@red@Disabled";
		getPA().sendFrame126("@or1@ Players Online: @gre@" + (PlayerHandler.getPlayers().size() + Config.PLAYERMODIFIER) ,10407);
		getPA().sendFrame126("@or1@ Player Rank = @gre@" + getRights().getPrimary().toString(), 10408);
		getPA().sendFrame126("@or1@ Daily Task: " + dailyStatus, 10409);
		getPA().sendFrame126("@or1@ Bonus XP: " + xpString, 10410);
		getPA().sendFrame126("@or1@ My KDR: @gre@"+ (double)(this.deathcount == 0 ? this.killcount + this.deathcount : this.killcount/this.deathcount), 10411);
		getPA().sendFrame126("@or1@ Time Played = @gre@"+time, 10412);
	...
	public void updateRedTab() {
		questTab = 2;
		getPA().sendFrame126("@or1@ Open NPC Droptable",10407);
		getPA().sendFrame126("@or1@ Open Guides", 10408);
		getPA().sendFrame126("", 10409);
		getPA().sendFrame126("@or1@ Player points:", 10410);
		getPA().sendFrame126("@or1@ Amount Donated: @gre@" + this.donatorPoints, 10411);
		getPA().sendFrame126("@or1@ Vote Points: @gre@" + this.votePoints, 10412);
		getPA().sendFrame126("@or1@ PK Points = @gre@" + this.pkp, 10413);
		getPA().sendFrame126("@or1@ Slayer Points = @gre@" + this.getSlayer().getPoints(), 10414); //m.getPointReward(0)
		getPA().sendFrame126("@or1@ Consecutive Tasks = @gre@" + this.getSlayer().getConsecutiveTasks(), 10415);
		getPA().sendFrame126("@or1@ PC points = @gre@" + this.pcPoints, 10416);
		getPA().sendFrame126("@or1@ Shayzien points = @gre@" +this.shayPoints, 10417);
		getPA().sendFrame126("@or1@ Raids Points = @gre@" + this.raidPoints, 10418);
		getPA().sendFrame126("", 10419);
	}
	
	public void updatePurpleTab() {
		questTab = 3;
		getPA().sendFrame126("@or1@ Open Achievements",10407);
		getPA().sendFrame126("", 10408);
		getPA().sendFrame126("@or1@ Quests:", 10409);
		getPA().sendFrame126(" Coming soon...", 10410);
		getPA().sendFrame126(" Coming soon...", 10411);
		getPA().sendFrame126(" Coming soon...", 10412);
		getPA().sendFrame126(" Coming soon...", 10413);
	...
	public void teleportBoss() {
		teleportTab = 0;
		getPA().sendFrame126("Barrelchest", 65031);
		getPA().sendFrame126("Dagannoth Kings", 65032);
		getPA().sendFrame126("King Black Dragon", 65033);
		getPA().sendFrame126("Giant Mole", 65034);
		getPA().sendFrame126("Kalphite Queen", 65035);
		getPA().sendFrame126("God Wars", 65036);
		getPA().sendFrame126("Corporeal Beast", 65037);
		getPA().sendFrame126("Dagg Mother", 65038);
		getPA().sendFrame126("Kraken", 65039);
		getPA().sendFrame126("Zulrah", 65040);
		getPA().sendFrame126("Cerberus", 65041);
		getPA().sendFrame126("Thermo Devils", 65042);
		getPA().sendFrame126("Abyssal Sire", 65043);
		getPA().sendFrame126("Demonic Gorillas", 65044);
		getPA().sendFrame126("Lizard Shaman", 65045);
		getPA().sendFrame126("Vorkath", 65046);
		getPA().sendFrame126("Inferno", 65047);
		getPA().sendFrame126("", 65048);
		getPA().sendFrame126("", 65049);
		getPA().sendFrame126("", 65050);
		getPA().sendFrame126("", 65051);
		getPA().sendFrame126("", 65052);
		getPA().sendFrame126("", 65053);
		getPA().sendFrame126("", 65054);
		getPA().sendFrame126("", 65055);
		getPA().sendFrame126("", 65056);
		getPA().sendFrame126("", 65057);
		getPA().sendFrame126("", 65058);
		getPA().sendFrame126("", 65059);
		getPA().sendFrame126("", 65060);
		//getPA().sendNpcIdToDisplayPacket(100, 65099);
	}
	
	public void teleportMonster() {
		teleportTab = 1;
		getPA().sendFrame126("Rock Crabs", 65031);
		getPA().sendFrame126("Cows", 65032);
		getPA().sendFrame126("Yaks", 65033);
		getPA().sendFrame126("Bob's Island", 65034);
		getPA().sendFrame126("Desert Bandits", 65035);
		getPA().sendFrame126("Elf Warriors", 65036);
		getPA().sendFrame126("Dagannoths", 65037);
		getPA().sendFrame126("Mithril Dragons", 65038);
		getPA().sendFrame126("Slayer Tower", 65039);
		getPA().sendFrame126("Demonic Gorillas", 65040);
		getPA().sendFrame126("Taverly Dungeon", 65041);
		getPA().sendFrame126("Stronghold Cave", 65042);
		getPA().sendFrame126("Smoke Devils", 65043);
		getPA().sendFrame126("", 65044);
		getPA().sendFrame126("", 65045);
		getPA().sendFrame126("", 65046);
		getPA().sendFrame126("", 65047);
		getPA().sendFrame126("", 65048);
		getPA().sendFrame126("", 65049);
		getPA().sendFrame126("", 65050);
		getPA().sendFrame126("", 65051);
	...
	public void teleportWilderness() {
		teleportTab = 2;
		getPA().sendFrame126("West Dragons", 65031);
		getPA().sendFrame126("Mage Bank", 65032);
		getPA().sendFrame126("Dark Castle", 65033);
		getPA().sendFrame126("Hill Giants (Multi)", 65034);
		getPA().sendFrame126("Wilderness Agility Course", 65035);
		getPA().sendFrame126("Vet'ion", 65036);
		getPA().sendFrame126("Callisto", 65037);
		getPA().sendFrame126("Scorpia", 65038);
		getPA().sendFrame126("Venenatis", 65039);
		getPA().sendFrame126("Chaos Elemental", 65040);
		getPA().sendFrame126("Chaos Fanatic", 65041);
		getPA().sendFrame126("Crazy Archaeologist", 65042);
		getPA().sendFrame126("", 65043);
		getPA().sendFrame126("", 65044);
		getPA().sendFrame126("", 65045);
		getPA().sendFrame126("", 65046);
		getPA().sendFrame126("", 65047);
	...
	public void teleportSkilling() {
		teleportTab = 3;
		getPA().sendFrame126("Slayer Masters", 65031);
		getPA().sendFrame126("Lands End", 65032);
		getPA().sendFrame126("Skillers Cove", 65033);
		getPA().sendFrame126("Woodcutting Guild", 65034);
		getPA().sendFrame126("Farming Patches", 65035);
		getPA().sendFrame126("Gnome Agility", 65036);
		getPA().sendFrame126("Hunter", 65037);
		getPA().sendFrame126("Puro Puro", 65038);
		getPA().sendFrame126("Mining Guild", 65039);
		getPA().sendFrame126("", 65040);
		getPA().sendFrame126("", 65041);
		getPA().sendFrame126("", 65042);
		getPA().sendFrame126("", 65043);
		getPA().sendFrame126("", 65044);
		getPA().sendFrame126("", 65045);
		getPA().sendFrame126("", 65046);
	...
	public void teleportMinigames() {
		teleportTab = 4;
		getPA().sendFrame126("Raids", 65031);
		getPA().sendFrame126("Warriors Guild", 65032);
		getPA().sendFrame126("Pest Control", 65033);
		getPA().sendFrame126("Fight Caves", 65034);
		getPA().sendFrame126("Barrows", 65035);
		getPA().sendFrame126("Clan Wars", 65036);
		getPA().sendFrame126("Shayzien Assault", 65037);
		getPA().sendFrame126("Mage Arena", 65038);
		getPA().sendFrame126("Duel Arena", 65039);
		getPA().sendFrame126("", 65040);
		getPA().sendFrame126("", 65041);
		getPA().sendFrame126("", 65042);
		getPA().sendFrame126("", 65043);
	...
	public void teleportCities() {
		teleportTab = 5;
		getPA().sendFrame126("Home", 65031);
		getPA().sendFrame126("Varrock", 65032);
		getPA().sendFrame126("Edgeville", 65033);
		getPA().sendFrame126("Yanille", 65034);
		getPA().sendFrame126("Lumbridge", 65035);
		getPA().sendFrame126("Ardougne", 65036);
		getPA().sendFrame126("Neitizknot", 65037);
		getPA().sendFrame126("Karamja", 65038);
		getPA().sendFrame126("Falador", 65039);
		getPA().sendFrame126("Taverly", 65040);
		getPA().sendFrame126("Camelot", 65041);
		getPA().sendFrame126("Catherby", 65042);
		getPA().sendFrame126("Al Kharid", 65043);
		getPA().sendFrame126("", 65044);
		getPA().sendFrame126("", 65045);
		getPA().sendFrame126("", 65046);
		getPA().sendFrame126("", 65047);
	...
	public void logout() {
		...
		if (System.currentTimeMillis() - logoutDelay > 100) {
			Hunter.abandon(this, null, true);
	...
	public void process() {
		...
		else if (inWild() && Boundary.isIn(this, Boundary.WILDERNESS_UNDERGROUND)) {
			int modY = absY > 6400 ? absY - 6400 : absY;
			wildLevel = (((modY - 3520) / 8) + 1);
			if (Config.SINGLE_AND_MULTI_ZONES) {
				getPA().sendFrame126("@yel@Level: " + wildLevel, 199);
	...
	public final int[] BOWS = { 19481, 19478, 12788, 9185, 11785, 21012, 839, 845, 847, 851, 855, 859, 841, 843, 849,
			...
			4221, 4222, 4223, 4734, 6724, 20997, 22547 };
	...
	public int gamemode;
	public int chinMultiDmg, barrageMultiDmg = 0;
	...
	public int getGalvekDamageCounter() {
		return this.raidsDamageCounters[5];
	}
	
	public void setGalvekDamageCounter(int damage) {
	...
	public TeleportTabTeleHandler getTeleportTab() {
	...
	public GalvekBag getGalvekBag() {
		return this.galvekBag;
	}
