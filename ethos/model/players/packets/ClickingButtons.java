...
import ethos.model.content.achievement_diary.kandarin.*;
...
public class ClickingButtons implements PacketType {
	...
	public void processPacket(final Player c, int packetType, int packetSize) {
		...
		if (c.getTutorial().isActive()) {
			if(actionButtonId == 132014) {
				c.getPA().sendFrame126("Standard playing style. Enjoy playing Anguish with", 33815);
				c.getPA().sendFrame126("the default combat and skilling rates!", 33816);
				c.getPA().sendFrame126("Perfect for learners!", 33817);
			...
			if(actionButtonId == 132017) {
				if(c.gamemode == 0) {
					c.setMode(Mode.forType(ModeType.REGULAR));
					c.sendMessage("You have selected normal gamemode! Enjoy Anguish!");
				} else if(c.gamemode == 1) {
					c.sendMessage("You have selected Ironman gamemode! Enjoy Anguish!");
					c.setMode(Mode.forType(ModeType.IRON_MAN));
					c.getRights().setPrimary(Right.IRONMAN);
				} else {
					c.sendMessage("You have selected Ultimate Ironman gamemode! Enjoy Anguish!");
		...
		if(Teleports.isTeleButton(c, actionButtonId)) {
			Teleports.teleportButtons(c, actionButtonId);
			c.getTeleport().handleTeleports(c, actionButtonId);
			
			
			c.getTeleportTab().handleTeleport(c, actionButtonId);
		}
		...
		case 108012:
			if(c.getName().equalsIgnoreCase("James") || c.getName().equalsIgnoreCase("Rowley")) {
				int offset = 9999;
