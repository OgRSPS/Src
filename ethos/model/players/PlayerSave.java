...
public class PlayerSave {
	...
	public static boolean playerExists(String name) {
		Path path = Paths.get("./data/saves/characters/", name.toLowerCase() + ".txt");
	...
	public static int loadGame(Player p, String playerName, String playerPass) {
		...
		boolean File1 = false;
		try {
			characterfile = new BufferedReader(new FileReader("./data/saves/characters/" + playerName.toLowerCase() + ".txt"));
			File1 = true;
				...
				case 2:
					if (token.equals("character-height")) {
						...
						if (!p.getMacAddress().equalsIgnoreCase(token2)) {
							PlayerLogging.write(LogType.CHANGE_MAC_ADDRESS, p,
									"Mac Address Changed: previous = " + token2 + ", new = " + p.getMacAddress());
						}
						p.setMacAddress(p.getMacAddress());
					} else if (token.equals("character-ip-address")) {
						if (!p.getIpAddress().equalsIgnoreCase(token2)) {
							PlayerLogging.write(LogType.CHANGE_IP_ADDRESS, p,
									"Ip Address Changed: previous = " + token2 + ", new = " + p.getIpAddress());
						}
					...
					} else if (token.equals("hasSacrificedFcape")) {
						p.hasSacrificedFcape = Boolean.parseBoolean(token2);
						...
						for (String claim : claimedRaw) {
							EntryDifficulty.forString(claim)
									.ifPresent(diff -> p.getDiaryManager().getVarrockDiary().claim(diff));
						}
					} else if (token.equals("ArdougneClaimedDiaries")) {
						String[] claimedRaw = token2.split(",");
						for (String claim : claimedRaw) {
							EntryDifficulty.forString(claim)
									.ifPresent(diff -> p.getDiaryManager().getArdougneDiary().claim(diff));
						}
					} else if (token.equals("DesertClaimedDiaries")) {
						String[] claimedRaw = token2.split(",");
						for (String claim : claimedRaw) {
							EntryDifficulty.forString(claim)
									.ifPresent(diff -> p.getDiaryManager().getDesertDiary().claim(diff));
						}
					} else if (token.equals("FaladorClaimedDiaries")) {
						String[] claimedRaw = token2.split(",");
						for (String claim : claimedRaw) {
							EntryDifficulty.forString(claim)
									.ifPresent(diff -> p.getDiaryManager().getFaladorDiary().claim(diff));
						}
					} else if (token.equals("FremennikClaimedDiaries")) {
						String[] claimedRaw = token2.split(",");
						for (String claim : claimedRaw) {
							EntryDifficulty.forString(claim)
									.ifPresent(diff -> p.getDiaryManager().getFremennikDiary().claim(diff));
						}
					} else if (token.equals("KandarinClaimedDiaries")) {
						String[] claimedRaw = token2.split(",");
						for (String claim : claimedRaw) {
							EntryDifficulty.forString(claim)
									.ifPresent(diff -> p.getDiaryManager().getKandarinDiary().claim(diff));
						}
					} else if (token.equals("KaramjaClaimedDiaries")) {
						String[] claimedRaw = token2.split(",");
						for (String claim : claimedRaw) {
							EntryDifficulty.forString(claim)
									.ifPresent(diff -> p.getDiaryManager().getKaramjaDiary().claim(diff));
						}
					} else if (token.equals("LumbridgeClaimedDiaries")) {
						String[] claimedRaw = token2.split(",");
						for (String claim : claimedRaw) {
							EntryDifficulty.forString(claim)
									.ifPresent(diff -> p.getDiaryManager().getLumbridgeDraynorDiary().claim(diff));
						}
					} else if (token.equals("MorytaniaClaimedDiaries")) {
						String[] claimedRaw = token2.split(",");
						for (String claim : claimedRaw) {
							EntryDifficulty.forString(claim)
									.ifPresent(diff -> p.getDiaryManager().getMorytaniaDiary().claim(diff));
						}
					} else if (token.equals("WesternClaimedDiaries")) {
						String[] claimedRaw = token2.split(",");
						for (String claim : claimedRaw) {
							EntryDifficulty.forString(claim)
									.ifPresent(diff -> p.getDiaryManager().getWesternDiary().claim(diff));
						}
					} else if (token.equals("WildernessClaimedDiaries")) {
						String[] claimedRaw = token2.split(",");
						for (String claim : claimedRaw) {
							EntryDifficulty.forString(claim)
									.ifPresent(diff -> p.getDiaryManager().getWildernessDiary().claim(diff));
						}
						...
						try {
							...
							for (String comp : components) {
								...
								// Lumbridge
								Optional<LumbridgeDraynorDiaryEntry> lumbridge = LumbridgeDraynorDiaryEntry
										.fromName(comp);
						...
						try {
							for (String comp : components) {
								...
								if (varrock.isPresent()) {
									p.getDiaryManager().getVarrockDiary().setAchievementStage(varrock.get(), stage,
											false);
								}
								// Ardougne
								Optional<ArdougneDiaryEntry> ardougne = ArdougneDiaryEntry.fromName(part[0]);
								if (ardougne.isPresent()) {
									p.getDiaryManager().getArdougneDiary().setAchievementStage(ardougne.get(), stage,
											false);
								}
								// Desert
								Optional<DesertDiaryEntry> desert = DesertDiaryEntry.fromName(part[0]);
								if (desert.isPresent()) {
									p.getDiaryManager().getDesertDiary().setAchievementStage(desert.get(), stage,
											false);
								}
								// Falador
								Optional<FaladorDiaryEntry> falador = FaladorDiaryEntry.fromName(part[0]);
								if (falador.isPresent()) {
									p.getDiaryManager().getFaladorDiary().setAchievementStage(falador.get(), stage,
											false);
								}
								// Fremennik
								Optional<FremennikDiaryEntry> fremennik = FremennikDiaryEntry.fromName(part[0]);
								if (fremennik.isPresent()) {
									p.getDiaryManager().getFremennikDiary().setAchievementStage(fremennik.get(), stage,
											false);
								}
								// Kandarin
								Optional<KandarinDiaryEntry> kandarin = KandarinDiaryEntry.fromName(part[0]);
								if (kandarin.isPresent()) {
									p.getDiaryManager().getKandarinDiary().setAchievementStage(kandarin.get(), stage,
											false);
								}
								// Karamja
								Optional<KaramjaDiaryEntry> karamja = KaramjaDiaryEntry.fromName(part[0]);
								if (karamja.isPresent()) {
									p.getDiaryManager().getKaramjaDiary().setAchievementStage(karamja.get(), stage,
											false);
								}
								// Lumbridge
								Optional<LumbridgeDraynorDiaryEntry> lumbridge = LumbridgeDraynorDiaryEntry
										.fromName(part[0]);
								if (lumbridge.isPresent()) {
									p.getDiaryManager().getLumbridgeDraynorDiary().setAchievementStage(lumbridge.get(),
											stage, false);
								}
								// Morytania
								Optional<MorytaniaDiaryEntry> morytania = MorytaniaDiaryEntry.fromName(part[0]);
								if (morytania.isPresent()) {
									p.getDiaryManager().getMorytaniaDiary().setAchievementStage(morytania.get(), stage,
											false);
								}
								// Western
								Optional<WesternDiaryEntry> western = WesternDiaryEntry.fromName(part[0]);
								if (western.isPresent()) {
									p.getDiaryManager().getWesternDiary().setAchievementStage(western.get(), stage,
											false);
								}
								// Wilderness
								Optional<WildernessDiaryEntry> wilderness = WildernessDiaryEntry.fromName(part[0]);
								if (wilderness.isPresent()) {
									p.getDiaryManager().getWildernessDiary().setAchievementStage(wilderness.get(),
											stage, false);
								}
							}
						} catch (Exception e) {
					...
					} else if (token.equals("herb-patch 0")) { // Tried removing the numbers and adding a loop (didn't
																// work?)
	...
	public static boolean saveGame(Player p) {
		...
		try {
			characterfile = new BufferedWriter(new FileWriter(Config.CHARACTER_SAVE_DIRECTORY + p.playerName.toLowerCase() + ".txt"));
			...
			characterfile.newLine();
			characterfile.write("hasSacrificedFcape = " + p.hasSacrificedFcape);
			characterfile.newLine();
			 ...
			 * p.lastConnectedFrom.get(i).length()); characterfile.newLine(); }
			 */
			...
			characterfile.newLine();
			characterfile.write("master-clue-reqs = " + p.masterClueRequirement[0] + "\t" + p.masterClueRequirement[1]
					+ "\t" + p.masterClueRequirement[2] + "\t" + p.masterClueRequirement[3]);
			characterfile.newLine();
			...
			characterfile.write(partialDiary, 0, partialDiary.length()); // Saw that earlier but forgot lol, ahh ty
			{
				...
				String prefix = "";
				// Varrock
				for (Entry<VarrockDiaryEntry, Integer> keyval : p.getDiaryManager().getVarrockDiary()
						.getPartialAchievements().entrySet()) {
				...
				// Ardougne
				for (Entry<ArdougneDiaryEntry, Integer> keyval : p.getDiaryManager().getArdougneDiary()
						.getPartialAchievements().entrySet()) {
				...
				// Desert
				for (Entry<DesertDiaryEntry, Integer> keyval : p.getDiaryManager().getDesertDiary()
						.getPartialAchievements().entrySet()) {
				...
				// Falador
				for (Entry<FaladorDiaryEntry, Integer> keyval : p.getDiaryManager().getFaladorDiary()
						.getPartialAchievements().entrySet()) {
				...
				// Fremennik
				for (Entry<FremennikDiaryEntry, Integer> keyval : p.getDiaryManager().getFremennikDiary()
						.getPartialAchievements().entrySet()) {
				...
				// Kandarin
				for (Entry<KandarinDiaryEntry, Integer> keyval : p.getDiaryManager().getKandarinDiary()
						.getPartialAchievements().entrySet()) {
				...
				// Karamja
				for (Entry<KaramjaDiaryEntry, Integer> keyval : p.getDiaryManager().getKaramjaDiary()
						.getPartialAchievements().entrySet()) {
				...
				// Lumbridge
				for (Entry<LumbridgeDraynorDiaryEntry, Integer> keyval : p.getDiaryManager().getLumbridgeDraynorDiary()
						.getPartialAchievements().entrySet()) {
				...
				// Morytania
				for (Entry<MorytaniaDiaryEntry, Integer> keyval : p.getDiaryManager().getMorytaniaDiary()
						.getPartialAchievements().entrySet()) {
				...
				// Western
				for (Entry<WesternDiaryEntry, Integer> keyval : p.getDiaryManager().getWesternDiary()
						.getPartialAchievements().entrySet()) {
				...
				// Wilderness
				for (Entry<WildernessDiaryEntry, Integer> keyval : p.getDiaryManager().getWildernessDiary()
						.getPartialAchievements().entrySet()) {
			...
			characterfile.write(Integer.toString(p.totalRaidsFinished), 0,
					Integer.toString(p.totalRaidsFinished).length());
			characterfile.newLine();
			...
			String toWrite55 = p.voidStatus[0] + "\t" + p.voidStatus[1] + "\t" + p.voidStatus[2] + "\t"
					+ p.voidStatus[3] + "\t" + p.voidStatus[4];
			...
			characterfile.newLine();
			characterfile.write("pouch-rune = " + p.getRuneEssencePouch(0) + "\t" + p.getRuneEssencePouch(1) + "\t"
					+ p.getRuneEssencePouch(2));
			characterfile.newLine();
			characterfile.write("pouch-pure = " + p.getPureEssencePouch(0) + "\t" + p.getPureEssencePouch(1) + "\t"
					+ p.getPureEssencePouch(2));
			characterfile.newLine();
			...
			for (int i = 0; i < Farming.MAX_PATCHES; i++) {
				characterfile.write("herb-patch " + i + " = " + p.getFarmingState(i) + "\t" + p.getFarmingSeedId(i)
						+ "\t" + p.getFarmingTime(i) + "\t" + p.getOriginalFarmingTime(i) + "\t"
						+ p.getFarmingHarvest(i));
				characterfile.newLine();
			}
			...
			for (int i = 0; i < p.halloweenRiddleGiven.length; i++)
				characterfile
						.write("" + p.halloweenRiddleGiven[i] + ((i == p.halloweenRiddleGiven.length - 1) ? "" : "\t"));
			characterfile.newLine();
			characterfile.write("halloweenOrderChosen = ");
			for (int i = 0; i < p.halloweenRiddleChosen.length; i++)
				characterfile.write(
						"" + p.halloweenRiddleChosen[i] + ((i == p.halloweenRiddleChosen.length - 1) ? "" : "\t"));
			characterfile.newLine();
			characterfile.write("halloweenOrderNumber = ", 0, 23);
			characterfile.write(Integer.toString(p.halloweenOrderNumber), 0,
					Integer.toString(p.halloweenOrderNumber).length());
			characterfile.newLine();
			...
			for (int i = 0; i < p.playerLevel.length; i++) {
				...
				characterfile.write("	", 0, 1);
				characterfile.write(Integer.toString(p.prestigeLevel[i]), 0,
						Integer.toString(p.prestigeLevel[i]).length());
				characterfile.newLine();
			}
			characterfile.newLine();
