	...
	public static void addCombatXP(Player c, CombatType type, int damage) {
		...
		case MELEE:
			...
			case 0: // Accurate
				...
				//c.sendMessage("You are OSRS: " + c.getMode().getType().equals(ModeType.OSRS) + " XP : " + (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.MELEE_EXP_RATE)) / 3);
				break;
			case 1: // Block
		...
		case RANGE:
			...
			case 3:
				skills.add(Skill.RANGED.getId());
				skills.add(Skill.DEFENCE.getId());
				experience += (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.RANGE_EXP_RATE)) + (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 16 : Config.RANGE_EXP_RATE) / 2);
				c.getPA().addSkillXP(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.RANGE_EXP_RATE / 2), 1, false);
		...
		case MAGE:
			...
			experience += ((damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.MAGIC_EXP_RATE)) / 3) + (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 16 : Config.MAGIC_EXP_RATE));
