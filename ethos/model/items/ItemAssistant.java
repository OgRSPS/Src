...
public class ItemAssistant {
	...
	public void sendWeapon(int Weapon, String WeaponName) {
		...
		if (WeaponName.equals("Unarmed")) {
			...
			c.getPA().sendFrame126(WeaponName, 8463);
				 ...
				 c.playerEquipment[c.playerWeapon] == 22978 
				|| WeaponName2.toLowerCase().contains("Dragon hunter lance") 
				|| WeaponName2.toLowerCase().contains("zamorakian") 
				|| WeaponName2.toLowerCase().contains("zamorakian hasta")) {
		...
		} else if (WeaponName2.contains("warhammer") || c.playerEquipment[c.playerWeapon] == 4153 || c.playerEquipment[c.playerWeapon] == 13263 || c.playerEquipment[c.playerWeapon] == 12848 || c.playerEquipment[c.playerWeapon] == 21003) {
	...
	public boolean is2handed(String itemName, int itemId) {
		...
		if (itemName.contains("godsword") || itemName.contains("crystal") || itemName.contains("aradomin sword")
				...
				itemName.contains("clueless") || itemName.contains("ballista") || itemName.contains("hunting knife") || itemName.contains("elder maul") || itemName.contains("bulwark") || itemName.contains("claws") || itemName.contains("vitur")) {
			return true;
		}
		switch (itemId) {
		case 12926:
	...
	public boolean wearItem(int wearID, int slot) {
		...
		if (c.playerItems[slot] == (wearID + 1)) {
			...
			/*c.sendMessage("Item ID: " + wearID);
			c.sendMessage("Slot: " + slot);
			c.sendMessage("Item: " + item);*/
			if (item == null) {
				if (wearID == 15098) {
					return false;
				}
			...
			switch (wearID) {
				...
				case 20997:
					if(c.playerLevel[4] < 75) {
						c.sendMessage(" You must have a ranged level of at least 75 to weild the Twisted Bow.");
						return false;
					}
					break;
				case 19478:
			...
			if (targetSlot == -1 || !item.isWearable()) {
				if (wearID >= 5509 && wearID <= 5512 || wearID == 21347 || wearID == 15098 || wearID == 11918 || wearID == 13656 || wearID == 7959 || wearID == 7960) {
					return false;
				} else if(wearID == 11941) {
					 c.getLootingBag().openWithdrawalMode();
				} else {
				c.sendMessage("This item cannot be worn." + wearID);
				return false;
				}
			}
			if (!canWearItem) {
			...
			if (slot >= 0 && wearID >= 0) {
				...
				stackable=getItemName(toRemove).contains("javelin")||getItemName(toRemove).contains("dart")||getItemName(toRemove).contains("knife")
						...
						||getItemName(toRemove).contains("Chin")||getItemName(toRemove).contains("chin");
	...
	public void removeFromBank(int itemId, int itemAmount, boolean updateView) {
		//c.sendMessage("THIS IS HOWIE DO IT IN BANK METHODS");
		BankTab tab = c.getBank().getCurrentBankTab();
		BankItem item = new BankItem(itemId + 1, itemAmount);
		boolean noted = false;
	...
	public String getTotalWorthSTR() {
		int worth = 0;
		String invValStr = Integer.toString(worth);
		for (int inventorySlot = 0; inventorySlot < c.playerItems.length; inventorySlot++) {
			int inventoryId = c.playerItems[inventorySlot] - 1;
			int inventoryAmount = c.playerItemsN[inventorySlot];
			int price = ShopAssistant.getItemShopValue(inventoryId);
		...
		invValStr = Integer.toString(worth);
		if(worth >= 1000000) { //millies
			invValStr = String.format("@gre@%.2fM", worth/ 1000000.0);
		}
		if(worth >= 1000000000) { //billies
			invValStr = String.format("@yel@%.2fB", worth/ 1000000000.0);
		}
